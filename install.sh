...
## ---------------------------------------
##
## Script name : Templates files installer
## Description : Script to install (sym-link), uninstall (unlink) the templates files to the system
...
# Source my loglib library
source "${_SCRIPT_DIR}/Templates/bash/lib/loglib.sh"
# Source my utils library
source "${_SCRIPT_DIR}/Templates/bash/lib/utilslib.sh"
...
# Use the XDG_TEMPLATES_DIR variable is setup, else make it default to $HOME/Templates
...
function main {
    ...
    create_folder "$( dirname $XDG_TEMPLATES_DIR )"

    # uninstall the script!
    if [[ $UNINSTALL = 1 ]]; then
        # TODO Uninstalling
        _log sec "Uninstalling '$XDG_TEMPLATES_DIR'..."
        # Remove links
        link_rm "$XDG_TEMPLATES_DIR"
        exit
    fi

    # TODO Installing
    _log sec "Installing '$XDG_TEMPLATES_DIR'..."
    # Create links
    link_create "Templates" "$XDG_TEMPLATES_DIR"
}
